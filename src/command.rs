    ...
    match parts[0] {
        ...
        "pano" => {
            ...
            if !app.is_panopticum_repo {
                return CommandAction::Error("Not a Panopticum repo (no PANOPTICUM.kdl found)".to_string());
            }
            
            // Check if panoctl is available
            if !crate::panopticum::is_panoctl_available(app.panoctl_binary_path.as_deref()) {
                return CommandAction::Error("panoctl binary not found in PATH. Install panoctl first.".to_string());
            }
            
            if parts.len() < 2 {
                return CommandAction::Error("Usage: :pano <validate|plan|apply> [env]".to_string());
            }
            ...
            match parts[1] {
                "validate" => {
                    ...
                    app.pano_status = crate::panopticum::PanoStatus::Running("Validating...".into());
                ...
                "plan" => {
                    ...
                    app.pano_status = crate::panopticum::PanoStatus::Running(format!("Planning {}...", env));
                ...
                "status" => {
                    ...
                    match &app.pano_status {
                        crate::panopticum::PanoStatus::Idle => CommandAction::Status("🔱 Panopticum: Idle".to_string()),
                        crate::panopticum::PanoStatus::Running(msg) => CommandAction::Status(format!("🔱 {}", msg)),
                        crate::panopticum::PanoStatus::Success(msg) => CommandAction::Status(format!("✓ {}", msg)),
                        crate::panopticum::PanoStatus::Error(msg) => CommandAction::Error(format!("✗ {}", msg)),
                        crate::panopticum::PanoStatus::OutputLine(line) => CommandAction::Status(line.clone()),
                    }
                }
                _ => CommandAction::Error(format!("Unknown pano command: {}. Use validate|plan|status", parts[1]))
            }
        },
