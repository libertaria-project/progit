...
fn trust_add(key_source: &str) -> Result<()> {
    ...
    if key_source.starts_with("https://") || key_source.starts_with("http://") {
        ...
        let response =
            reqwest::blocking::get(key_source).context("Failed to fetch key from URL")?;

        let pk_bytes = response
            .bytes()
        ...
        println!(
            "  {} Using ProGit Core Team test key: {}",
            "✓".green(),
            keyid
        );
...
pub fn handle_plugin_verify(name: &str) -> Result<()> {
    ...
    if result.valid {
        ...
        if result
            .errors
            .iter()
            .any(|e| e.contains("not in trusted keyring"))
        {
            println!("\n  Add the publisher key to trust it:");
            println!(
                "  prog trust add https://registry.progit.dev/keys/{}.pub",
                result.keyid
            );
        }
