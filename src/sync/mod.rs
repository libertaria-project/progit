    ...
    for remote_issue in remote_issues {
        if let Some(remote_id) = remote_issue.remotes.get(provider_name) {
            ...
            if let Some(existing_idx) = merged
                .iter()
                .position(|i| i.remotes.get(provider_name) == Some(remote_id))
            {
                ...
                if remote_issue.updated > local_updated {
                    ...
                    log::info!(
                        "⬇ Updating local issue '{}' from remote #{} (remote newer)",
                        local_title,
                        remote_id
                    );
                    ...
                    if !merged[existing_idx].remotes.contains_key(provider_name) {
                        merged[existing_idx]
                            .remotes
                            .insert(provider_name.to_string(), remote_id.clone());
                    }
                    log::debug!(
                        "⬆ Keeping local changes for '{}' (local newer or equal)",
                        local_title
                    );
                }
            } else if let Some(title_match_idx) = merged.iter().position(|i| {
                i.title
                    .trim()
                    .eq_ignore_ascii_case(remote_issue.title.trim())
            }) {
                // Match by Title (loose) - link them
                log::info!(
                    "🔗 Linking local '{}' to remote #{} by title match",
                    merged[title_match_idx].title,
                    remote_id
                );

                // Add remote link to local issue
                merged[title_match_idx]
                    .remotes
                    .insert(provider_name.to_string(), remote_id.clone());
            } else {
                ...
                if !merged
                    .iter()
                    .any(|i| i.remotes.get(provider_name) == Some(remote_id))
                {
            ...
            // merged.push(remote_issue);
            log::warn!(
                "⚠️ Skipping remote issue without ID: '{}'",
                remote_issue.title
            );
        }
    }
