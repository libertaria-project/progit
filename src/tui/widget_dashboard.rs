...
pub fn render(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let header = Paragraph::new(header_text).block(Block::default().borders(Borders::NONE));
    ...
    let issue_stats = Paragraph::new(issue_stats_text).block(
        Block::default()
            .borders(Borders::ALL)
            .title(" Issue Health "),
    );
    ...
    let open_mrs = app
        .mr_list
        .iter()
        .filter(|m| m.state == crate::mr::MRState::Open)
        .count();
    let merged_mrs = app
        .mr_list
        .iter()
        .filter(|m| m.state == crate::mr::MRState::Merged)
        .count();
    ...
    let mr_stats = Paragraph::new(mr_stats_text).block(
        Block::default()
            .borders(Borders::ALL)
            .title(" Forge Status "),
    );
    ...
    let shortcuts_text = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("   [Tab]  ", colors.warning()),
            Span::raw(" Cycle View Modes"),
            Span::styled("       [n]    ", colors.warning()),
            Span::raw(" New Issue"),
        ]),
        Line::from(vec![
            Span::styled("   [S]    ", colors.warning()),
            Span::raw(" Sync with Forge"),
            Span::styled("       [O]    ", colors.warning()),
            Span::raw(" Settings"),
        ]),
        Line::from(vec![
            Span::styled("   [/]    ", colors.warning()),
            Span::raw(" Global Search"),
            Span::styled("       [q]    ", colors.warning()),
            Span::raw(" Quit"),
        ]),
        Line::from(vec![
            Span::styled("   [Ctrl+P]", colors.warning()),
            Span::raw(" Command Palette"),
            Span::styled("     [d]    ", colors.warning()),
            Span::raw(" Quick Diff"),
        ]),
    ];
    let shortcuts = Paragraph::new(shortcuts_text).block(
        Block::default()
            .borders(Borders::ALL)
            .title(" Quick Start "),
    );
