        ...
        for (name, config) in &self.config_styles {
            if let Some(parent) = &config.inherits {
                if self.has_circular_inheritance(name, parent) {
                    return Err(format!("Circular inheritance detected: {} -> {}", name, parent));
                }
            }
        }
        ...
        for (name, config) in &self.config_styles {
            if let Some(fg) = &config.fg {
                if parse_color(fg).is_none() {
                    return Err(format!("Invalid foreground color '{}' in style '{}'", fg, name));
                }
            }
            if let Some(bg) = &config.bg {
                if parse_color(bg).is_none() {
                    return Err(format!("Invalid background color '{}' in style '{}'", bg, name));
                }
            }
        }
        ...
        for (name, config) in &self.config_styles {
            for modifier in &config.modifiers {
                if !self.is_valid_modifier(modifier) {
                    return Err(format!("Invalid modifier '{}' in style '{}'", modifier, name));
                }
            }
        }
...
fn resolve_inheritance(
    ...
    let result = if let Some(parent_name) = &config.inherits {
        if let Some(parent_config) = all_styles.get(parent_name) {
            ...
            let resolved_parent = resolve_inheritance(parent_name, parent_config, all_styles, visited);
...
mod tests {
    ...
    fn test_inheritance_resolution() {
        ...
        // Base style
        configs.insert("base".to_string(), StyleConfig {
        ...
        configs.insert("child".to_string(), StyleConfig {
    ...
    fn test_cycle_detection() {
        ...
        // Cycle: A -> B -> A
        configs.insert("A".to_string(), StyleConfig {
            fg: Some("red".to_string()),
            bg: None, modifiers: vec![],
            inherits: Some("B".to_string()),
        });
        
        configs.insert("B".to_string(), StyleConfig {
    ...
    fn test_validate() {
        ...
        // Invalid color
        configs.insert("bad_color".to_string(), StyleConfig {
            fg: Some("not_a_color".to_string()),
            bg: None, modifiers: vec![], inherits: None,
        });

        // Valid style
        configs.insert("good".to_string(), StyleConfig {
            fg: Some("red".to_string()),
            bg: None, modifiers: vec![], inherits: None,
        });
