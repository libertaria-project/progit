...
pub fn render(frame: &mut Frame, area: Rect, app: &mut App) -> Option<Rect> {
    ...
    let left_content = if let Some(ref repo) = app.repo_info {
        ...
        info.push(Span::styled(
            " │ ",
            engine.get("status.separator", colors.dim()),
        ));
        ...
        info.push(Span::styled(
            short_path,
            engine.get("status.path", colors.dim()),
        ));
        ...
        Line::from(vec![
            ...
            Span::styled(
                " No git repository",
                engine.get("status.norepo", colors.dim()),
            ),
        ])
    };
        ...
        _ => {
            ...
            if let Some(ref msg) = status_msg {
                Line::from(Span::styled(
                    format!(" {}", msg),
                    engine.get("status.message", colors.warning()),
                ))
            } else {
                ...
                Line::from(vec![
                    Span::styled(
                        ...
                        engine.get("status.clock", colors.accent()),
                    ),
...
pub fn help_text(app: &App) -> String {
    match app.view_mode {
        crate::tui::ViewMode::Dashboard => "Tab:list  S:sync  O:settings  q:quit".to_string(),
        ...
        crate::tui::ViewMode::Diff => "j/k:scroll  J/K:files  space:collapse  q:close".to_string(),
